...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, features='default', normalize=False, assign_feat='default', max_num_nodes=0):
        ...
        for G in G_list:
            ...
            if normalize:
                sqrt_deg = np.diag(1.0 / np.sqrt(np.sum(adj, axis=1, dtype=float).squeeze()))
        ...
        if features == 'deg-num':
            self.feature_all = self.feature_all / maxdeg
            self.assign_feat_all = self.assign_feat_all / maxdeg
